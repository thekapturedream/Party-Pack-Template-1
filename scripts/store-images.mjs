// Renders the shop listing images. They reuse the kit's own design tokens so
// the listing and the product read as one thing, and they show real pages
// rather than a styled photograph of something we did not make.
...
const sheetCss = await readFile('src/design/sheet.css', 'utf8');
const fonts = await readFile('assets/fonts/fonts.css', 'utf8');

const png = async (name) =>
  `data:image/png;base64,${(await readFile(`dist/preview/${name}.png`)).toString('base64')}`;
...
<style>${fonts}</style><style>${tokens}</style><style>${sheetCss}</style>
<style>
  body { width:${w}px; height:${h}px; margin:0; background:var(--paper-tint); }
  .canvas { width:${w}px; height:${h}px; padding:64px; display:flex; flex-direction:column; }
...
for (const img of IMAGES) {
  ...
  // A JPEG copy as well: the shop upload path is size-sensitive and these are
  // flat-colour compositions that compress cleanly.
  await page.screenshot({ path: `${OUT}/${img.name}.jpg`, type: 'jpeg', quality: 88 });
  await page.close();
  console.log(`${OUT}/${img.name}.png`);
}
await browser.close();
